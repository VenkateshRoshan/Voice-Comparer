...
'''
# on_disabled : True if filechooser.selection==[] else False
class KitchenSink(App):
    ...
    def open_FileManager(self , cur) :
        if cur == 'Inserting_Screen' :
            self.root.ids.filechooser.filters = ['*.aac','*.wav']
    ...
    def Add_to_DB(self,text) :
        filename = text
        db = mysql.connector.connect(host = 'localhost' , user = 'root' , passwd = 'P@$$W0RD')
        my = db.cursor()
    ...
    def Insert_DB(self,StudId,File,show_label):
        ...
        if num_labels > 1 :
            ...
            #model.add(Dropout(.3))
            model.add(layers.Dense(num_labels,kernel_initializer=activation_fn))
            ...
            model.compile(loss='sparse_categorical_crossentropy',
                          ...
                          metrics=['accuracy'])
            model.summary()
            model.fit(Train_audio,Train_label,epochs=epochs,validation_split=.1)
